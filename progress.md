...
## 2026-07-01 (저녁) — 점주 주문관리 UX 4건 (빠른 접수/완료/취소 + admin 모바일 네비·로그아웃 + 시간표시 버그)

세션 흐름: 사용자가 점주 주문화면·admin 헤더 관련 4건 지목 → 코드 → Playwright(umaidon, 임시 PENDING 주문 삽입/삭제)로 검증.

- **① 목록에서 바로 접수/완료/취소** (`owner.orders.tsx` `QuickActions`): 알림톡 "주문 확인하기"로 들어온 점주가 상세 안 열고 목록에서 바로 처리. **PENDING→[접수]**(손님 요청 픽업시간 그대로 확정+고객 확정 알림톡) / **진행중(ACCEPT·PREPARING·READY)→[완료]/[취소]**. 픽업 조정·거절(사유 안내)만 행 클릭→상세. 버튼은 `stopPropagation`으로 행 클릭(상세 열기)과 분리. 데스크톱 그리드 상태칸 col-span 2→3(메뉴 4→3), 모바일 카드 하단 버튼행.
- **④ 취소=상태만 변경(무통지)**: 목록 취소는 `silent=true` → action의 `notifyCustomer`에서 제외(노쇼·별도연락 취소용). 상세의 거절/취소(사유 select)는 기존대로 고객 알림. **상태전환 완화**: ACCEPT·PREPARING·READY에서 COMPLETED/CANCEL 직접 허용(`STATUS_TRANSITIONS`) — 점주가 중간단계 생략하고 완료/취소 가능.
- **④ 시간표시 버그 수정**: 목록의 `fmtKST(iso).split(" ")[1]`이 ko-KR 포맷(`2026. 7. 1. 23:00:00`) 때문에 **시간 대신 "7."(월)**을 뽑던 버그. `fmtKstDate`(7월 1일)·`fmtKstTime`(오후 11:00) 명시적 헬퍼 신설, 목록 시간·픽업·상세 헤더 전부 교체.
- **② admin 모바일 네비**: 헤더 `메뉴/주문/설정` 링크가 `hidden lg:flex`라 모바일에서 사라지던 것 → `flex`(상시 노출). 설정은 가게정보 섹션으로 스크롤(anchor href="#" 무동작 → 버튼 onClick scrollIntoView).
- **③ admin 로그아웃 버튼**: 오른쪽 위 노란 **카카오 말풍선(chat_bubble) 아이콘이 사실 로그아웃**이라 헷갈림 → `logout` 아이콘+**"로그아웃" 텍스트** 버튼으로 교체(다른 페이지와 통일).
- (참고) owner.orders 우상단 말풍선은 프로필 아바타의 카카오 연동 **장식 배지**(로그아웃 아님) → 그대로 둠. 로그아웃은 사이드바 "로그아웃".
- **검증(Playwright umaidon)**: 임시 PENDING 주문(260701-02) 삽입 → 목록에 [접수], 기존 PREPARING(260701-01)에 [완료]/[취소], 시간 "오전 11:32/10:48"·"픽업 확정 오전 11:10" 정상, admin 모바일 네비 3개+로그아웃 텍스트, chat_bubble 제거 확인 → 임시주문 삭제. typecheck·build 통과.
